...
public class Tienda {
  ...
  */
  public static void colocarComic(Comic nuevoComic) { 
    for(int i = 0; i < almacenComics.length; i++) {
        if(almacenComics[i] == null) {
  ...
  */
  public static void mostrarComics() {
    for(Comic comic: almacenComics) { //Recorrido For Each, para cada objeto Comic en el arreglo de objetos Comic(almacenComis)
  ...
  */
  public static String [][] obtenerDatos() {
    ...
    for (Comic comic : almacenComics) //Recorrido del arreglo de Comics, para cada comic en almacenComics
    {
